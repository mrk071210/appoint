 ...
 * found in the LICENSE file at https://angular.io/license
 */
function createLanguageService(host){return new LanguageServiceImpl(host)}function uniqueBySpan(elements){if(elements){for(var result=[],map=new Map,_i=0,elements_1=elements;_i<elements_1.length;_i++){var element=elements_1[_i],span=element.span,set=map.get(span.start);set||(set=new Set,map.set(span.start,set)),set.has(span.end)||(set.add(span.end),result.push(element))}return result}}function findSuitableDefaultModule(modules){for(var result,resultSize=0,_i=0,_a=modules.ngModules;_i<_a.length;_i++){var module_1=_a[_i],moduleSize=module_1.transitiveModule.directives.length;moduleSize>resultSize&&(result=module_1,resultSize=moduleSize)}return result}function createLanguageServiceFromTypescript(host,service){var ngHost=new TypeScriptServiceHost(host,service),ngServer=createLanguageService(ngHost);return ngHost.setSite(ngServer),ngServer}function typeCallable(type){var signatures=type.getCallSignatures();return signatures&&0!=signatures.length}function signaturesOf(type,context){return type.getCallSignatures().map(function(s){return new SignatureWrapper(s,context)})}function selectSignature(type,context,types){var signatures=type.getCallSignatures();return signatures.length?new SignatureWrapper(signatures[0],context):void 0}function toSymbolTable(symbols){for(var result={},_i=0,symbols_1=symbols;_i<symbols_1.length;_i++){var symbol=symbols_1[_i];result[symbol.name]=symbol}return result}function toSymbols(symbolTable,filter){var result=[];if(!symbolTable)return result;var own="function"==typeof symbolTable.hasOwnProperty?function(name){return symbolTable.hasOwnProperty(name)}:function(name){return!!symbolTable[name]};for(var name_1 in symbolTable)!own(name_1)||filter&&!filter(symbolTable[name_1])||result.push(symbolTable[name_1]);return result}function findClassSymbolInContext(type,context){var sourceFile=context.program.getSourceFile(type.filePath);if(sourceFile){var moduleSymbol=sourceFile.module||sourceFile.symbol,exports_1=context.checker.getExportsOfModule(moduleSymbol);return(exports_1||[]).find(function(symbol){return symbol.name==type.name})}}function findTsConfig(fileName){for(var dir=require$$2.dirname(fileName);fs.existsSync(dir);){var candidate=require$$2.join(dir,"tsconfig.json");if(fs.existsSync(candidate))return candidate;var parentDir=require$$2.dirname(dir);if(parentDir===dir)break;dir=parentDir}}function isSymbolPrivate(s){return s.valueDeclaration&&isPrivate(s.valueDeclaration)}function getBuiltinTypeFromTs(kind,context){var type,checker=context.checker,node=context.node;switch(kind){case BuiltinType.Any:type=checker.getTypeAtLocation(setParents({kind:typescript.SyntaxKind.AsExpression,expression:{kind:typescript.SyntaxKind.TrueKeyword},type:{kind:typescript.SyntaxKind.AnyKeyword}},node));break;case BuiltinType.Boolean:type=checker.getTypeAtLocation(setParents({kind:typescript.SyntaxKind.TrueKeyword},node));break;case BuiltinType.Null:type=checker.getTypeAtLocation(setParents({kind:typescript.SyntaxKind.NullKeyword},node));break;case BuiltinType.Number:var numeric={kind:typescript.SyntaxKind.NumericLiteral};setParents({kind:typescript.SyntaxKind.ExpressionStatement,expression:numeric},node),type=checker.getTypeAtLocation(numeric);break;case BuiltinType.String:type=checker.getTypeAtLocation(setParents({kind:typescript.SyntaxKind.NoSubstitutionTemplateLiteral},node));break;case BuiltinType.Undefined:type=checker.getTypeAtLocation(setParents({kind:typescript.SyntaxKind.VoidExpression,expression:{kind:typescript.SyntaxKind.NumericLiteral}},node));break;default:throw new Error("Internal error, unhandled literal kind "+kind+":"+BuiltinType[kind])}return type}function setParents(node,parent){return node.parent=parent,typescript.forEachChild(node,function(child){return setParents(child,node)}),node}function spanOf$1(node){return{start:node.getStart(),end:node.getEnd()}}function shrink(span,offset){return null==offset&&(offset=1),{start:span.start+offset,end:span.end-offset}}function spanAt(sourceFile,line,column){if(null!=line&&null!=column){var position_1=typescript.getPositionOfLineAndCharacter(sourceFile,line,column),findChild=function findChild(node){if(node.kind>typescript.SyntaxKind.LastToken&&node.pos<=position_1&&node.end>position_1){var betterNode=typescript.forEachChild(node,findChild);return betterNode||node}},node=typescript.forEachChild(sourceFile,findChild);if(node)return{start:node.getStart(),end:node.getEnd()}}}function definitionFromTsSymbol(symbol){var declarations=symbol.declarations;if(declarations)return declarations.map(function(declaration){var sourceFile=declaration.getSourceFile();return{fileName:sourceFile.fileName,span:{start:declaration.getStart(),end:declaration.getEnd()}}})}function parentDeclarationOf(node){for(;node;){switch(node.kind){case typescript.SyntaxKind.ClassDeclaration:case typescript.SyntaxKind.InterfaceDeclaration:return node;case typescript.SyntaxKind.SourceFile:return null}node=node.parent}}function getContainerOf(symbol,context){if(symbol.getFlags()&typescript.SymbolFlags.ClassMember&&symbol.declarations)for(var _i=0,_a=symbol.declarations;_i<_a.length;_i++){var declaration=_a[_i],parent_1=parentDeclarationOf(declaration);if(parent_1){var type=context.checker.getTypeAtLocation(parent_1);if(type)return new TypeWrapper(type,context)}}}function getTypeParameterOf(type,name){if(type&&type.symbol&&type.symbol.name==name){var typeArguments=type.typeArguments;if(typeArguments&&typeArguments.length<=1)return typeArguments[0]}}function typeKindOf(type){if(type){if(type.flags&typescript.TypeFlags.Any)return BuiltinType.Any;if(type.flags&(typescript.TypeFlags.String|typescript.TypeFlags.StringLike|typescript.TypeFlags.StringLiteral))return BuiltinType.String;if(type.flags&(typescript.TypeFlags.Number|typescript.TypeFlags.NumberLike))return BuiltinType.Number;if(type.flags&typescript.TypeFlags.Undefined)return BuiltinType.Undefined;if(type.flags&typescript.TypeFlags.Null)return BuiltinType.Null;if(type.flags&typescript.TypeFlags.Union){var candidate=void 0,unionType=type;if(unionType.types.length>0){candidate=typeKindOf(unionType.types[0]);for(var _i=0,_a=unionType.types;_i<_a.length;_i++){var subType=_a[_i];if(candidate!=typeKindOf(subType))return BuiltinType.Other}}return candidate}if(type.flags&typescript.TypeFlags.TypeParameter)return BuiltinType.Unbound}return BuiltinType.Other}/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 ...
 * found in the LICENSE file at https://angular.io/license
 */
var STRIP_SRC_FILE_SUFFIXES=/(\.ts|\.d\.ts|\.js|\.jsx|\.tsx)$/,NG_FACTORY=/\.ngfactory\./,NgModuleResolver=function(){function NgModuleResolver(_reflector){void 0===_reflector&&(_reflector=reflector),this._reflector=_reflector}return NgModuleResolver.prototype.isNgModule=function(type){return this._reflector.annotations(type).some(_isNgModuleMetadata)},NgModuleResolver.prototype.resolve=function(type,throwIfNotFound){void 0===throwIfNotFound&&(throwIfNotFound=!0);var ngModuleMeta=findLast(this._reflector.annotations(type),_isNgModuleMetadata);if(ngModuleMeta)return ngModuleMeta;if(throwIfNotFound)throw new Error("No NgModule metadata found for '"+stringify(type)+"'.");return null},NgModuleResolver}();NgModuleResolver.decorators=[{type:CompilerInjectable}],NgModuleResolver.ctorParameters=function(){return[{type:ReflectorReader}]};var PipeResolver=function(){function PipeResolver(_reflector){void 0===_reflector&&(_reflector=reflector),this._reflector=_reflector}return PipeResolver.prototype.isPipe=function(type){var typeMetadata=this._reflector.annotations(resolveForwardRef(type));return typeMetadata&&typeMetadata.some(_isPipeMetadata)},PipeResolver.prototype.resolve=function(type,throwIfNotFound){void 0===throwIfNotFound&&(throwIfNotFound=!0);var metas=this._reflector.annotations(resolveForwardRef(type));if(metas){var annotation=findLast(metas,_isPipeMetadata);if(annotation)return annotation}if(throwIfNotFound)throw new Error("No Pipe decorator found on "+stringify(type));return null},PipeResolver}();PipeResolver.decorators=[{type:CompilerInjectable}],PipeResolver.ctorParameters=function(){return[{type:ReflectorReader}]};/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 ...
 * found in the LICENSE file at https://angular.io/license
 */
var VERSION$$1=new Version("4.0.3");exports.createLanguageService=createLanguageService,exports.create=create,exports.TypeScriptServiceHost=TypeScriptServiceHost,exports.createLanguageServiceFromTypescript=createLanguageServiceFromTypescript,exports.VERSION=VERSION$$1,Object.defineProperty(exports,"__esModule",{value:!0})});
//# sourceMappingURL=/usr/local/google/home/misko/angular/dist/packages-dist/language-service/bundles/language-service.umd.min.js.map