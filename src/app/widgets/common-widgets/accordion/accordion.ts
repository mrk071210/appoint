...
import {Header} from '../../commons/shared';
import {BlockableUI} from '../../commons/blockableui';
...
@Component({
    ...
    template: `
        <div class="ui-accordion-header ui-state-default ui-corner-all" [ngClass]="{'ui-state-active': selected,'ui-state-disabled':disabled}"
            (click)="toggle($event)">
            <span class="fa fa-fw" [ngClass]="{'fa-caret-down': selected&&canExpanded, 'fa-caret-right': !(selected&&canExpanded)}"></span>
        ...
        <div class="ui-accordion-content-wrapper" [@tabContent]="selected&&canExpanded ? 'visible' : 'hidden'" (@tabContent.done)="onToggleDone($event)"
            [ngClass]="{'ui-accordion-content-wrapper-overflown': !selected||animating}" role="tabpanel" [attr.aria-hidden]="!selected">
            <div class="ui-accordion-content ui-widget-content" *ngIf="lazy ? selected&&canExpanded : canExpanded">
                <ng-content></ng-content>
            </div>
        </div>
    `,
...
export class AccordionTab implements OnDestroy {
    ...
    @Input() canExpanded: boolean = true; // added by zhangyi at 2017年09月25日 控制某一条目是否可以展开折叠，默认为true；
