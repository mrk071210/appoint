...
@Component({
    ...
    template: `
        <div>
            <canvas [attr.width]="width" [attr.height]="height" (click)="onCanvasClick($event)" (dblclick)="onCanvasDblClick($event)"></canvas>
        </div>
    `
})
export class UIChart implements AfterViewInit, OnDestroy {
    ...
    //双击事件回调函数  wangshuang  2017/10/16
    onCanvasDblClick(event) {
        if(this.chart) {
            let element = this.chart.getElementAtEvent(event);
            let dataset = this.chart.getDatasetAtEvent(event);
            if(element&&element[0]&&dataset) {
                this.onDoubleClick.emit({originalEvent: event, element: element[0], dataset: dataset});
            }
        }
    }
